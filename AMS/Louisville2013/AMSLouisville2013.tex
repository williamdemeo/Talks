...
\titlegraphic{
\vskip-2cm
\includegraphics[height=.18\textheight]{aux/American_Mathematical_Society}
\hfill
\includegraphics[height=.18\textheight]{aux/UofLouisvilleMinervaSeal.png}
}
...
\includeonlyframes{titlepage,Motivation1,Motivation,LocalStructure,Parachutes0,Parachutes2,ExamplesPrezi,Examples,Example7elementPrezi,ASClass,IEProps,IEPropsLemma,IEPropsConjecture,IEPropsLemma2,L7second,IdeaOfL7Proof,FinalConclusions,IEPropsInsoluble}
...
\includegraphics[width=\paperwidth]{aux/A5}%
\end{frame}
...
\end{exs}
\vskip4mm
\uncover<2->{At the other extreme, there are finite lattices that are not subgroup lattices.}\\[6pt]
\uncover<2->{Example?}
\end{frame}
...
\begin{center}
 ...
 \draw
    ...
    \visible<5->{\shade[top color=olivegreen,bottom color=gray!50] 
      (U) to [out=-10,in=100] (U0) to [out=170,in=-80] (U);
      \draw (-7,0) node {$\lb U_0,U \rb:=\{V \mid U_0 \leq V \leq U\}$};}
    % \visible<6->{  \shade[top color=blue,bottom color=gray!50] 
    %   (UH) to [out=-10,in=100] (H) to [out=170,in=-80] (UH);
    %   \draw (7,6) node {$\lb H, \<U,H\>]$};}
...
\begin{frame}[fragile,label=IntervalIsomorphisms,shrink=5]{Interval Isomorphisms}
  \begin{columns}
    \begin{column}{0.3\textwidth}
      \begin{center}
        \begin{tikzpicture}[scale=.3]
          \input{aux/IntervalIsomorphisms.tex}
    ...
    \visible<1,4->{  \shade[top color=olivegreen,bottom color=gray!-45] 
      (UH) to [out=-10,in=100] (H) to [out=170,in=-80] (UH);}
    \visible<5->{\fill[color=\ConfColor]    
      (H) to [out=190,in=80] (U0) to [out=10,in=-100] (H);}
    \visible<5->{\shade[top color=\ConfColor,bottom color=gray!0]    
      (UH) to [out=190,in=80] (U) to [out=10,in=-100] (UH);}
       \end{tikzpicture}
      \end{center}
    \end{column}
    \begin{column}{0.8\textwidth}
      ...
      \item<1-> If $H\subnormal \<U, H\>$, then %\\[4pt]
        $UH = \<U, H\> \; \text{ and } \; \lb U_0, U \rb \cong \lb H, UH \rb$.\vskip2mm
  ...
  \visible<4->{
    \begin{lemma}
      ...
      \item $\lb H, UH \rb  \cong  \lb U_0, U \rb^H \leq \lb U_0, U \rb$ 
       % \only<6>{ and \hskip2mm $\lb U, UH \rb \cong  \lb U_0, H \rb^U \leq \lb U_0, H \rb$.}
\\[6pt]
      \item If $U \subnormal UH$, then  $\lb U_0, U \rb_H  = \lb U_0, U \rb^H \leq \lb U_0, U \rb$.\\[6pt]
...
\begin{frame}[fragile,label=ProofOfIntervalIsomorphism,shrink=5]{Proof of Interval Isomorphism Lemma}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{claim}[1]
      $\lb H, UH \rb  \cong  \lb U_0, U \rb^H$ via
        \begin{align*}
          \phi: \;& \lb H, UH \rb \ni X \mapsto U\cap X \in \lb U_0, U \rb^H\\[6pt]
...
\vskip5mm
  ...
  %%   \begin{column}{0.6\textwidth}
      \begin{proof}
        \begin{itemize}
        \item[1.] For $X\in \lb H, UH \rb$,
          check $U\cap X \in \lb U_0, U \rb^H$ by Dedekind's rule.\vskip6pt
        \item[2.] For $V\in \lb U_0, U \rb^H$, $VH$ is a group in $\lb H, UH \rb$.\vskip6pt
...
\begin{frame}[fragile,label=ProofOfIntervalIsomorphism,shrink=5]{Proof of Interval Isomorphism Lemma}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{claim}[2]
      $\lb U_0, U \rb^H$ is a sublattice of $\lb U_0,U \rb$.
      \end{claim}
    \end{column}
    \begin{column}{0.55\textwidth}
       \begin{center}
 ...
 %     \vskip1cm
  ...
  \end{columns}
       \begin{proof}
          Fix $V, W \in \lb U_0, U \rb^H$.\vskip6pt
          \begin{itemize}
...
\begin{frame}[label=IEProps]{2. What properties of $G$ can be inferred from $\lb H,G \rb$?}
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
A group theoretical property $\cP$  %(and the associated class $\sG_\cP$) 
is
\vskip2mm
\begin{itemize}
\item 
...
$L$ such that
\[
%\text{ if $G\in \G$ and $L\cong \lb H,G \rb$ \; \longrightarrow \; G \text{ is a%$\cP$-group.}
...
$\exists L$ st
\[
%L\cong \lb H,G \rb \text{ and $H$ core-free} \; \longrightarrow \; G \text{ is a $\cP$-group.}
...
%% provided $\exists L$ st\\ if $L\cong \lb H,G \rb$ for a group $G$ of
...
\defn{Nonsolvability} 
\vskip2mm
There exist finite lattices that cannot occur as
upper intervals in the subgroup lattices of finite \emph{solvable} groups.
\vskip4mm
\uncover<2->{
Here are a few
\vskip1cm
...
\frame[label=MO,shrink=5]{
  ...
  \begin{center}
    \includegraphics[height=2.8in]{aux/MO-20120921}
  \end{center}
}  
...
%% The three subgroups in $\lb H,G \rb$ which cover $H$ (the atoms) are
...
\end{center}
  ...
  %%   \begin{column}{0.7\textwidth}
      ...
      \item<4-> Since $J_1K = G$ and $J_1\cap K = H$, 
      \[
...
respectively:
\[
L\cong \lb H,G \rb \; \Longrightarrow \; \text{$G$ has property $\cP$}
\]
\[
L_c\cong \lb H_c,G_c\rb \; \Longrightarrow \; \text{$G_c$ does not have property $\cP$}
\]}
\visible<3->{ %Then this lattice would not be group representable.  
%\begin{center}
  \begin{tikzpicture}[scale=0.5]
    \input{aux/TwoPanel.tex}
    \draw (-5,5.2) node {Then the lattice};
    \draw (-.93,5.2) node {$L$};
    \draw (.93,5.2) node {$L_c$};
    \draw (8,4) node {could not be group representable.};
  \end{tikzpicture}
%\end{center}
}
\end{frame}
...
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
But if a group property and its
...
%% \vskip2mm
%% Then $G\times G_c$ has upper intervals 
%% $L\cong \lb H\times G_c, G\times G_c]$ and 
...
\visible<2->{
\begin{corollary}
Solvable is not a core-free interval enforceable property.
\end{corollary}
}
\visible<3->{
\begin{corollary}
Almost simple is not a core-free interval enforceable property.
\end{corollary}
}
\end{frame}
...
\vskip4mm
  ...
  \cong \lb H,G \rb$.
%% \vskip4mm
%% \visible<2->{ (So $G$ has property $\cP$.)}
\vskip4mm
\visible<3->{ Use Kurzweil's method for representing duals of lattices (twice):
\begin{columns}
    \begin{column}{0.2\textwidth}
  \begin{center}
\includegraphics[height=3cm]{aux/Kurzweil-crop}
  \end{center}
    \end{column}
    \begin{column}{0.7\textwidth}
  \begin{itemize}
...
The interval  $\lb D, S^n\rb$ is isomorphic to $\Eq(n)^*$, the dual of the lattice of
partitions of an $n$-element set.
\vskip4mm
  The dual lattice $L^*$ is an upper interval of $\Sub(U)$, namely,
  $L^*\cong \lb D\bar{G}, U \rb$. 
\end{frame}
...
Repeating the procedure, with $H_1 :=
  D\bar{G}$ and $L^* \cong \lb H_1, U \rb$, 
we find $L = L^{**}\cong \lb D_1 \bar{U}, S^m\bar{U} \rb$, where $m =
...
\vskip4mm
\visible<2->{So a class of groups that omits wreath products of the form $S\wr G$, where $S$ is an arbitrary finite nonabelian simple group, is not a core-free interval enforceable class.  
\vskip4mm
Examples: the class of solvable groups, the class of almost simple groups.}
\end{frame}
...
\visible<2->{
{\bf Questions:}\\[6pt]
1. We have only considered interval enforceable properties.\\
\hskip4mm What about property enforceable intervals?\\[6pt]
\visible<3->{
2. Could PE intervals be used to prove Nice Boring Theorems? (cf. Peter Mayr's talk)}
}
\end{frame}

\begin{frame}[fragile,label=Parachutes1]{}
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
\begin{center}
{\scalefont{.8}
...
\begin{frame}[fragile,label=Parachutes2]{}
%\ifthenelse{\boolean{prezi}}{\FontLarge}{}
